...
#
# light_attack_range:
# Enemy can use fast_poke when the player is this close.
#
...
@export var light_attack_range: float = 1.5
@export var hover_range_min: float = 1.7
@export var hover_range_max: float = 2.6
@export var far_range: float = 4.0

@export var approaching_heavy_min_range: float = 2.0
@export var approaching_heavy_max_range: float = 4.0
...
@export_range(0.0, 1.0, 0.01) var retreat_chance_too_close: float = 0.35
@export_range(0.0, 1.0, 0.01) var hover_hold_chance: float = 0.75
@export_range(0.0, 1.0, 0.01) var approaching_heavy_chance: float = 0.12
...
@export var shared_action_cooldown: float = 0.65
@export var light_attack_cooldown: float = 1.0
@export var heavy_attack_cooldown: float = 2.6
...
func _physics_process(delta: float) -> void:
	...
	match character.control_mode:
		...
		# You can later split AI_PROFILE_1 / 2 / 3 into different behaviours.
...
func _think(_delta: float) -> void:
	...
	# AI v0.1 does not guard by default.
	# If you enable can_guard in the Inspector later, the same profile can test it.
	...
	# Distance behaviour is deliberately simple:
	# far -> approach
	# hover -> hold/creep
	# close -> light attack
	# too close -> sometimes retreat
	if abs_depth <= too_close_range:
		_handle_too_close(abs_depth, dir_away_from_target)
		return

	if abs_depth <= light_attack_range:
		_handle_close(abs_depth)
		return

	if abs_depth >= approaching_heavy_min_range and abs_depth <= approaching_heavy_max_range:
...
func _handle_too_close(_abs_depth: float, dir_away_from_target: float) -> void:
	...
	_debug_state(&"TOO_CLOSE_HOLD")


func _handle_close(abs_depth: float) -> void:
	character.set_guarding(false)
	character.set_desired_lane_dir(0.0)
